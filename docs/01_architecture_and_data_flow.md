...
본 시스템은 외부 프레임워크와 인프라 인스턴스의 변경이 핵심 비즈니스 로직에 영향을 주지 않도록 **Clean Architecture (클린 아키텍처)** 패턴을 따르며, 의존성 역전 원칙(DIP, Dependency Inversion Principle)을 철저히 준수합니다.

### 🏗️ 레이어별 정의 및 Java Spring 매칭
* **Presentation Layer (`src/app`)**
  * **역할:** 외부의 HTTP 요청을 접수하고 최종 응답을 반환하는 진입점입니다.
  * **기술 스택:** FastAPI (패스트에이피아이), Uvicorn (유비콘)
  * **Java 매칭:** Spring Boot Controller (스프링 부트 컨트롤러)
* **Domain Layer (`src/domain`)**
  * **역할:** 어떠한 인프라(DB, AI 엔진 등)에도 의존하지 않는 순수한 비즈니스 규칙, 엔티티 상태, 최적화 알고리즘의 핵심 코어입니다.
...
## 2. 런타임 환경 및 동시성 정책 (Runtime & Concurrency)
...
## 3. 핵심 데이터 파이프라인 흐름 (Data Flow)
주간/월간 단위의 대용량 배치성 식단 최적화 요청 처리 시, 시스템의 I/O 비용을 최소화하고 성능을 극대화하기 위한 **In-Memory Batch Simulation (인메모리 배치 시뮬레이션)** 구조로 데이터가 흐릅니다.
